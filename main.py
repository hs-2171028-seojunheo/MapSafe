...
def generate_explanation_with_gemini(score: float, features: dict) -> str:
    ...
    2. 첫 문장 이후 줄바꿈(<br><br>)을 한 번 하고, 이 거리가 왜 해당 점수를 받았는지 최대 3~4문장 분량으로 분석 이유를 이어서 작성하세요.
    3. [등급 반영] 현재 점수가 [안전 점수별 등급 기준] 중 어디에 해당하는지 파악하고, 이 거리가 안전한 상태인지, 보통인지, 혹은 위험 요소를 내포하고 있는지 문맥 속에 자연스럽게 단어를 녹여내어 브리핑하세요.
    4. [차량 맥락 분석] 일반 차량의 '숫자(많고 적음)'만으로 안전도를 기계적으로 판단하지 마세요. 주어진 다른 지표(도로 폭 등)들과 최종 점수를 종합하여, 차량이 현재 보행 환경에 어떤 맥락으로 존재하는지(예: 보행 동선 간섭 여부, 차량 운행 상태 등)를 입체적으로 분석하세요.
    5. [차종 엄격 구분] '일반 차량'과 '대형 트럭/화물차'는 반드시 엄격하게 분리하여 판단하세요.
    6. 주어진 기준을 바탕으로 분석하되, 마치 전문가가 거리를 직접 보고 자연스럽게 환경을 묘사하듯이 문장을 구성하세요. (예: "가로수 비율이 높아 전반적으로 안전하고 쾌적한 보행 환경을 제공합니다. 다만, 대형 트럭이 발견되어 보통 수준의...")
    7. 보행자나 차량 등의 개수를 언급할 때는 소수점이 아닌 반드시 정수(자연수)로만 표현하세요. (예: 2.0대 -> 2대)
    8. 응답은 웹사이트에 바로 렌더링할 수 있도록 HTML 태그를 사용해주세요. 문단 구분에 <br>을 활용하고, 강조하고 싶은 핵심 명사나 특징에는 <b> 태그를 사용하세요.
    """
